...
struct Args {
    /// config
    #[clap(short = 't', long = "theta_min", num_args(1..), value_name="theta min")]
    theta_min: f64,
    ...
    #[clap(short = 'p', long = "phi_min", value_name = "phi_min")]
    phi_min: f64,

    #[clap(short = 'P', long = "phi_max", value_name = "phi_max")]
    phi_max: f64,

    #[clap(short = 'N', long = "nphi", value_name = "num of phi")]
    nphi: usize,

    #[clap(short = 'f', long = "freq_min", value_name = "freq min")]
    freq_min: f64,

    #[clap(short = 'F', long = "freq_max", value_name = "freq_max")]
    freq_max: f64,

    #[clap(short = 'm', long = "nfreq", value_name = "num of freq")]
    nfreq: usize,
    ...
    #[clap(short = 'a', long = "az0", value_name = "phase center az")]
    az0: f64,

    #[clap(short = 'c', long = "cfg", value_name = "array_cfg.yaml")]
    cfg: String,

    #[clap(short = 'A', long = "antenna_beam", value_name = "ant beam in healpix")]
    ant_beam: String,
...
fn main() {
    ...
    let mut output_fits=FitsFile::create(&args.outfile).with_custom_primary(&image_description).open().unwrap();
    ...
    for f_idx in 0..args.nfreq{
        ...
        let lambda=C/freq;
        let phases=calc_phase_from_pointing(&ant_x, &ant_y, &ant_z, args.az0.to_radians(), args.zenith0.to_radians(), lambda);
        //println!("{:?}", phases);
        for phi_idx in 0..args.nphi{
            ...
            for theta_idx in 0..args.ntheta{
                ...
                buf[(row_idx, 0)]=theta;
                buf[(row_idx, 1)]=phi;
                buf.row_mut(row_idx).as_slice_mut().unwrap().chunks_exact_mut(2).skip(1).zip(ant_jones.iter()).for_each(|(a,&b)|{
                    let g=array_beam*b;
                    //let g=array_beam;
                    a[0]=g.re;
                    a[1]=g.im;
